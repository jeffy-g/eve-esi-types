...
export function defineColors() {
    Object.defineProperties(String.prototype, {
        ...
        green: {
            get() {
                return `<span style="color: green">${this}</span>`;
            }
        },
        yellow: {
            get() {
                return `<span style="color: yellow">${this}</span>`;
            }
        },
        blue: {
            get() {
                return `<span style="color: blue">${this}</span>`;
            }
        },
        magenta: {
            get() {
                return `<span style="color: #FF00FF">${this}</span>`;
            }
        },
        cyan: {
            get() {
                return `<span style="color: #00FFFF">${this}</span>`;
            }
        },
    });
}
...
const jsonBeautify = (jsonString) => {
    const re = /("(?:\\[\s\S]|[^"])*"(?:\s*:)?)|(-?\d+(?:\.\d*)?(?:[eE][+\-]?\d+)?)|\b(true|false)\b|\b(null)\b/gm;
    ...
    if (isNode) {
        ...
        const log = (...data) => {
            if (logElement) {
                ...
                requestAnimationFrame(() => logElement.innerHTML += text + "\n");
            }
        };
        return log;
    }
