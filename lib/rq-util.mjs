...
export const is = (opt) => {
    if (isNode) {
        ...
        if (q) {
            ...
            /*/
            const usp = new URLSearchParams(q.slice(1));
...
export const initOptions = (method, opt) => {
    ...
    // 2026/03/03 14:10:54
    if (opt.compatDate) { // valid "X-Compatibility-Date" date string or empty string, undefined
        // headers["X-Compatibility-Date"] = opt.compatDate;
        // NOTE: avoid preflight request
...
export const curl = (endpoint) => {
    ...
    // return `${BASE}/latest/${(/** @type {string} */(endpoint) as string).replace(/^\/+|\/+$/g, "")}/`;
};
/**
...
export async function fireRequestsDoesNotRequireAuth(fn) {
    ...
    try {
        ...
        // 2026/03/03 17:06:29 - PLEX new endpoint
        // https://esi.evetech.net/markets/19000001/orders/?language=en&order_type=all&type_id=44992
